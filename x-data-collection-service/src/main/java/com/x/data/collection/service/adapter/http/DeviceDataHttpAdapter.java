...
import com.x.data.collection.service.utils.parser.DataParseService;
...
public class DeviceDataHttpAdapter {
    ...
    public R<String> receiveDeviceData(
            @PathVariable(name = "deviceCode", required = false) String deviceCode,
        ...
        CompletableFuture.runAsync(() -> {
            try {
                // 将数据发送到Kafka
                kafkaProducerService.sendMessageAsync(deviceCode, JSONObject.toJSONString(dataParseService.parse(JSONObject.toJSONString(data,SERIALIZE_FEATURES),deviceCode),SERIALIZE_FEATURES));
