package com.x.data.collection.tcp.service.config;
...
public class BusinessThreadPoolConfig {
     ...
     * 核心业务线程池（处理 TCP 异步业务）
     */
    @Bean(name = "tcpBusinessExecutor")
    public Executor tcpBusinessExecutor() {
        ...
        executor.setThreadNamePrefix("tcp-business-"); // 线程名前缀（方便日志排查）

        // CallerRunsPolicy：让调用线程（Netty IO线程）临时处理任务，避免任务丢失（牺牲部分IO性能，换可用性）
